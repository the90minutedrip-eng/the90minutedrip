...
const CinematicIntro: React.FC<CinematicIntroProps> = ({ onComplete, isTransitioning = false }) => {
  ...
  useEffect(() => {
    ...
    const loadThreeJS = async () => {
      try {
        ...
        console.warn('Three.js failed to load, using fallback intro:', error);
        setThreeError(true);
        // Show brand text immediately and auto-complete after 3 seconds
        setShowBrandText(true);
        timeoutIdRef.current = window.setTimeout(skipIntro, 3000);
      }
    };

    const initializeThreeJS = (THREE: any) => {
  ...
  return (
    <div className={`fixed inset-0 z-50 transition-opacity duration-1000 ease-out ${isTransitioning ? 'opacity-0' : 'opacity-100'}`}>
      ...
      {/* Fallback background when Three.js fails */}
      {threeError && (
        <div 
